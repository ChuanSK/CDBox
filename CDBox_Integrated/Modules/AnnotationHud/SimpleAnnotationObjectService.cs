...
namespace TCPipeAutoDraw.Modules.AnnotationHud
{
    internal static class SimpleAnnotationObjectService
    {
            ...
            out ObjectId annotationObjectId)
        {
            ...
            using (Transaction tr = doc.Database.TransactionManager.StartTransaction())
            {
                foreach (ObjectId id in selectedIds)
                {
                    ...
                    if (text != null && (LooksLikeSurfaceText(text.TextString)
                        || LooksLikeNodeText(text.TextString)
        ...
        {
            if (selected == null) return null;
            if (LooksLikeSurfaceText(selected.TextString))
            {
                var model = CreateBaseModel(db, tr, selected, SimpleAnnotationKind.SurfaceArea);
                model.AnnotationId = Guid.NewGuid().ToString("D");
                model.Header = "表面积标注";
                model.Summary = "旧注记 · 保存后纳入统一浮窗管理";
                model.Lines.Add(new SimpleAnnotationTextLine
                {
                    ObjectId = selected.ObjectId,
                    Role = "MainText",
                    Label = "注记文字",
                    Text = selected.TextString ?? string.Empty,
                    Color = CDBoxColorService.FromCadColor(selected.Color)
                });
                Entity leader = FindLegacySurfaceLeader(db, tr, selected);
                if (leader != null)
                {
                    model.LeaderObjectId = leader.ObjectId;
                    model.SecondaryColor = CDBoxColorService.FromCadColor(leader.Color);
                    model.LinetypeName = ReadLinetypeName(tr, leader.LinetypeId);
                    model.LineWeight = leader.LineWeight;
                    Curve curve = leader as Curve;
                    if (curve != null) model.AnchorPoint = SafeCurveStart(curve, model.AnchorPoint);
                }
                return model;
            }
        ...
        private static Entity FindLegacySurfaceLeader(Database db, Transaction tr, DBText text)
        {
            Point3d anchor = GetTextAnchor(text);
            double tolerance = Math.Max(text.Height * 2.0, 0.2);
            Entity best = null;
            double bestDistance = double.MaxValue;
            foreach (ObjectId id in EnumerateCurrentSpace(db, tr))
            {
                Polyline line;
                try { line = tr.GetObject(id, OpenMode.ForRead, false) as Polyline; }
                catch { continue; }
                if (line == null || line.NumberOfVertices < 2
                    || !string.Equals(line.Layer, text.Layer, StringComparison.OrdinalIgnoreCase)) continue;
                Point3d end = line.GetPoint3dAt(line.NumberOfVertices - 1);
                double distance = end.DistanceTo(anchor);
        ...
        private static bool LooksLikeSurfaceText(string text)
        {
            string value = text ?? string.Empty;
            return value.IndexOf("面积", StringComparison.OrdinalIgnoreCase) >= 0
                && (value.IndexOf("㎡", StringComparison.OrdinalIgnoreCase) >= 0
                    || value.IndexOf("m2", StringComparison.OrdinalIgnoreCase) >= 0

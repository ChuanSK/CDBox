...
namespace CDBox.RealEstate.UI
{
    internal static class ParcelSurveyExportDialog
    {
        ...
        public static string ExportProperty(ParcelSurveyRecord record)
        {
            var dialog=new SaveFileDialog {Title="导出房产文档",Filter="Word 文档 (*.docx)|*.docx",
                DefaultExt=".docx",AddExtension=true,FileName=BuildingPropertyWordExporter.DefaultFileName(record)};
            SetInitialDirectory(dialog);
            if(dialog.ShowDialog()!=true)return string.Empty;
            BuildingPropertyWordExporter.Export(dialog.FileName,record);return dialog.FileName;
        }

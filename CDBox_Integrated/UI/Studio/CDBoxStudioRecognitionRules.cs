...
{
    internal static class CDBoxStudioRecognitionRules
    {
        ...
        public static List<LayerRecognitionRule> LoadRules()
        {
            ...
            {
                CDBoxStudioLogger.Error("读取属性识别表失败，已回退到默认表。", ex);
        ...
        public static List<LayerRecognitionRule> LoadDefaultRules()
        {
            ...
            {
                CDBoxStudioLogger.Error("读取默认属性识别表失败。", ex);
        ...
        public static int SavePayload(string payload)
        {
            ...
            CDBoxStudioLogger.Info("Studio 已保存属性识别表。规则数量：" + rules.Count + "，路径：" + RulesFilePath);
            return rules.Count;
        }

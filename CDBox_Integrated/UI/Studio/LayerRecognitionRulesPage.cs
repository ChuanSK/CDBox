...
.recognition-rules-page .rules-table tr.dragging{opacity:.4}.recognition-rules-page .rules-table tr.drag-target td{background:rgba(59,130,246,.10)}.recognition-rules-page .rr-drag{cursor:grab;user-select:none}.recognition-rules-page .rr-drag:active{cursor:grabbing;color:var(--brand)}
...
");
            css.Append(".recognition-rules-page .rr-card,.recognition-rules-page .rules-table-wrap,.recognition-rules-page .rr-table-wrap{border-radius:12px}.recognition-rules-page .empty-line,.recognition-rules-page .rr-error>div{border-radius:12px}");
            css.Append(".recognition-rules-page .rules-table{min-width:1920px}.recognition-rules-page .rules-table td:nth-child(2),.recognition-rules-page .rules-table td:nth-child(13){text-align:center;width:76px}.recognition-rules-page .rules-table td:nth-child(3){width:170px}.recognition-rules-page .rules-table td:nth-child(4){width:90px}.recognition-rules-page .rules-table td:nth-child(5){width:105px}.recognition-rules-page .rules-table td:nth-child(6){width:120px}.recognition-rules-page .rules-table td:nth-child(7){width:260px}.recognition-rules-page .rules-table td:nth-child(8){width:210px;text-align:left}.recognition-rules-page .rules-table td:nth-child(9){width:130px}.recognition-rules-page .rules-table td:nth-child(10){width:160px}.recognition-rules-page .rules-table td:nth-child(11){width:260px}.recognition-rules-page .rules-table td:nth-child(12){width:120px}.recognition-rules-page .rules-table input[type=number]{width:82px;border:1px solid var(--line);border-radius:8px;background:var(--panel);color:var(--text);padding:7px 8px;outline:0}.recognition-rules-page .rr-test{margin-top:10px;border:1px solid var(--line);border-radius:12px;background:var(--panel2);padding:10px 12px}.recognition-rules-page .rr-test summary{cursor:pointer;font-size:13px;font-weight:800;color:var(--text)}.recognition-rules-page .rr-test-grid{display:grid;grid-template-columns:minmax(240px,1fr) minmax(320px,1.5fr);gap:10px;margin:10px 0}.recognition-rules-page .rr-test textarea,.recognition-rules-page .rr-test pre{min-height:90px;max-height:150px;margin:0;border:1px solid var(--line);border-radius:9px;background:var(--panel);color:var(--text);padding:9px;overflow:auto;white-space:pre-wrap;font:12px/1.55 'Microsoft YaHei UI','Segoe UI',sans-serif}.recognition-rules-page .rr-test textarea{resize:vertical}.recognition-rules-page .rr-test .rr-btn{height:34px}@media(max-width:900px){.recognition-rules-page .rr-test-grid{grid-template-columns:1fr}}");
            if (standalone)
            {
        ...
        private static string BuildEditorCard(bool standalone, string logFilePath)
        {
            ...
            page.Append("<div class=\"rr-toolbar\"><div class=\"rr-left\"><div class=\"rr-search-wrap\" id=\"searchWrap\"><input id=\"rulesSearch\" class=\"rr-search\" placeholder=\"搜索：规则、匹配、排除、输出属性...\" /><button id=\"toggleSearch\" class=\"rr-search-icon\" title=\"搜索 / Ctrl+F\">🔍</button></div><button id=\"addRuleButton\" class=\"rr-btn\">新增</button><button id=\"deleteRuleButton\" class=\"rr-btn danger\">删除</button><button id=\"saveRulesButton\" class=\"rr-btn primary\">保存</button></div></div>");
            page.Append("<div class=\"rules-table-wrap rr-table-wrap\"><table class=\"rules-table\"><thead><tr><th>排序</th><th>启用</th><th>规则名称</th><th>优先级</th><th>范围</th><th>匹配方式</th><th>匹配内容</th><th>排除条件</th><th>父属性</th><th>分类</th><th>标签</th><th>合并方式</th><th>命中停止</th></tr></thead><tbody id=\"rulesBody\"></tbody></table></div>");
            page.Append("<div id=\"rulesEmpty\" class=\"empty-line\">没有匹配的规则。</div>");
            page.Append("<details class=\"rr-test\"><summary>规则快速测试</summary><div class=\"rr-test-grid\"><textarea id=\"ruleTestInput\" placeholder=\"每行一个图层名，例如：&#10;300波纹管（砼恢复）&#10;井-沉泥井-D700-砖砌-铸铁盖\"></textarea><pre id=\"ruleTestOutput\">输入图层名后点击“测试当前规则”。结构化属性、置信度与冲突请在图层管理器扫描结果中查看。</pre></div><button id=\"testRulesButton\" class=\"rr-btn\">测试当前规则</button></details>");
            page.Append("</article>");
            return page.ToString();
        }
        ...
        private static string BuildComponentScript()
        {
            return @"
(function(w){
  function cloneRules(source){return (source||[]).map(function(r){return {id:r.id||'',name:r.name||'',enabled:!!r.enabled,priority:Number(r.priority)||100,scope:r.scope||'图层名',matchMode:r.matchMode||'通配符',pattern:r.pattern||'',excludePattern:r.excludePattern||'',parentGroup:r.parentGroup||'',parentClass:r.parentClass||'',tagText:r.tagText||'',mergeMode:r.mergeMode||'覆盖空值',applicableObjectTypes:r.applicableObjectTypes||'',source:r.source||'用户规则',confidenceBase:Number(r.confidenceBase)||.78,stopAfterMatch:!!r.stopAfterMatch};});}
  function b64(value){return btoa(unescape(encodeURIComponent(value||'')));}
  function RecognitionRulesPage(options){this.options=options||{};this.root=document.getElementById(this.options.rootId||'recognitionRulesPage');if(!this.root)throw new Error('未找到属性识别表页面根节点。');this.rules=cloneRules(this.options.data);this.defaultRules=cloneRules(this.options.defaultData);this.selectedIndex=this.rules.length?0:-1;this.matchModes=['精确','包含','通配符','关键词组合','模板','正则'];this.mergeModes=['覆盖空值','覆盖','首条命中'];this.bound=false;}
  ...
  RecognitionRulesPage.prototype.ruleText=function(rule){return ((rule.name||'')+' '+(rule.scope||'')+' '+(rule.matchMode||'')+' '+(rule.pattern||'')+' '+(rule.excludePattern||'')+' '+(rule.parentGroup||'')+' '+(rule.parentClass||'')+' '+(rule.tagText||'')).toLowerCase();};
  ...
  RecognitionRulesPage.prototype.numberInput=function(value,min,max,step,oninput){var input=document.createElement('input');input.type='number';input.value=value;input.min=min;input.max=max;input.step=step;input.addEventListener('click',function(ev){ev.stopPropagation();});input.addEventListener('input',function(){var n=Number(input.value);if(isFinite(n))oninput(n);});return input;};
  ...
  RecognitionRulesPage.prototype.valueSelect=function(values,value,onchange){var select=document.createElement('select');var current=value||values[0];select.addEventListener('click',function(ev){ev.stopPropagation();});values.forEach(function(item){var option=document.createElement('option');option.value=item;option.textContent=item;if(item===current)option.selected=true;select.appendChild(option);});select.addEventListener('change',function(){onchange(select.value);});return select;};
  RecognitionRulesPage.prototype.bindRowDrag=function(row,index){var self=this,handle=row.querySelector('.rr-drag');if(handle){handle.draggable=true;handle.title='拖拽排序';handle.addEventListener('click',function(ev){ev.stopPropagation();});handle.addEventListener('dragstart',function(ev){self.dragIndex=index;row.classList.add('dragging');if(ev.dataTransfer)ev.dataTransfer.effectAllowed='move';});handle.addEventListener('dragend',function(){self.dragIndex=null;row.classList.remove('dragging','drag-target');});}row.addEventListener('dragover',function(ev){if(self.dragIndex==null||self.dragIndex===index)return;ev.preventDefault();row.classList.add('drag-target');});row.addEventListener('dragleave',function(){row.classList.remove('drag-target');});row.addEventListener('drop',function(ev){if(self.dragIndex==null||self.dragIndex===index)return;ev.preventDefault();var from=self.dragIndex,box=row.getBoundingClientRect(),to=index+(ev.clientY>box.top+box.height/2?1:0),item=self.rules.splice(from,1)[0];if(from<to)to--;to=Math.max(0,Math.min(self.rules.length,to));self.rules.splice(to,0,item);self.selectedIndex=to;self.dragIndex=null;self.render();});};
  RecognitionRulesPage.prototype.render=function(){var body=this.$('#rulesBody');if(!body)return;var search=this.$('#rulesSearch');var q=((search&&search.value)||'').trim().toLowerCase();body.innerHTML='';var visible=0;var self=this;this.rules.forEach(function(rule,index){if(q&&self.ruleText(rule).indexOf(q)<0)return;visible++;var row=document.createElement('tr');row.className=index===self.selectedIndex?'selected':'';row.addEventListener('click',function(){self.selectedIndex=index;self.render();});var order=self.cell(row,'⋮⋮ '+String(index+1));order.className='rr-drag';self.cell(row).appendChild(self.check(rule.enabled,function(v){rule.enabled=v;}));self.cell(row).appendChild(self.textInput(rule.name,function(v){rule.name=v;}));self.cell(row).appendChild(self.numberInput(rule.priority,1,9999,1,function(v){rule.priority=Math.round(v);}));self.cell(row).appendChild(self.textInput(rule.scope,function(v){rule.scope=v;}));self.cell(row).appendChild(self.modeSelect(rule.matchMode,function(v){rule.matchMode=v;}));self.cell(row).appendChild(self.textInput(rule.pattern,function(v){rule.pattern=v;}));self.cell(row).appendChild(self.textInput(rule.excludePattern,function(v){rule.excludePattern=v;}));self.cell(row).appendChild(self.textInput(rule.parentGroup,function(v){rule.parentGroup=v;}));self.cell(row).appendChild(self.textInput(rule.parentClass,function(v){rule.parentClass=v;}));self.cell(row).appendChild(self.textInput(rule.tagText,function(v){rule.tagText=v;}));self.cell(row).appendChild(self.valueSelect(self.mergeModes,rule.mergeMode,function(v){rule.mergeMode=v;}));self.cell(row).appendChild(self.check(rule.stopAfterMatch,function(v){rule.stopAfterMatch=v;}));self.bindRowDrag(row,index);body.appendChild(row);});var empty=this.$('#rulesEmpty');if(empty)empty.style.display=visible?'none':'block';};
  RecognitionRulesPage.prototype.addRule=function(){this.rules.push({id:'',name:'新规则',enabled:true,priority:100,scope:'图层名',matchMode:'通配符',pattern:'',excludePattern:'',parentGroup:'',parentClass:'',tagText:'',mergeMode:'覆盖空值',applicableObjectTypes:'',source:'用户规则',confidenceBase:.78,stopAfterMatch:true});this.selectedIndex=this.rules.length-1;this.render();};
  ...
  RecognitionRulesPage.prototype.buildPayload=function(){return this.rules.map(function(r){return [r.enabled?'1':'0',String(Number(r.priority)||100),r.name||'',r.scope||'图层名',r.matchMode||'通配符',r.pattern||'',r.excludePattern||'',r.parentGroup||'',r.parentClass||'',r.tagText||'',r.mergeMode||'覆盖空值',r.applicableObjectTypes||'',r.source||'用户规则',String(Number(r.confidenceBase)||.78),r.stopAfterMatch?'1':'0'].map(b64).join('\t');}).join('\n');};
  RecognitionRulesPage.prototype.matches=function(rule,name){if(!rule.enabled||!rule.pattern)return false;var n=String(name||'').replace(/\s+/g,''),p=String(rule.pattern||'').replace(/\s+/g,''),ex=String(rule.excludePattern||'').split(/[，,、;；|]/).filter(Boolean);if(ex.some(function(x){return n.toLowerCase().indexOf(x.trim().toLowerCase())>=0;}))return false;try{if(rule.matchMode==='精确')return n.toLowerCase()===p.toLowerCase();if(rule.matchMode==='包含')return n.toLowerCase().indexOf(p.toLowerCase())>=0;if(rule.matchMode==='正则')return new RegExp(rule.pattern,'i').test(name);if(rule.matchMode==='关键词组合')return p.split(/[，,、;；|]/).filter(Boolean).every(function(x){return n.toLowerCase().indexOf(x.trim().toLowerCase())>=0;});var rx='^'+p.replace(/[.+^${}()|[\]\\]/g,'\\$&').replace(/\*/g,'.*').replace(/\?/g,'.')+'$';if(rule.matchMode==='模板')rx='^'+p.replace(/[.+^$()|[\]\\]/g,'\\$&').replace(/\\?\{DN\}/g,'(?:DN)?\\d+(?:\\.\\d+)?').replace(/\\?\{D\}/g,'D\\d+(?:\\.\\d+)?').replace(/\\?\{T\}/g,'T\\d+(?:\\.\\d+)?').replace(/\\?\{(?:Material|Type)\}/g,'.+?').replace(/\*/g,'.*')+'$';return new RegExp(rx,'i').test(n);}catch(ex2){return false;}};
  RecognitionRulesPage.prototype.testRules=function(){var input=this.$('#ruleTestInput'),output=this.$('#ruleTestOutput');if(!input||!output)return;var self=this,names=(input.value||'').split(/\r?\n/).map(function(x){return x.trim();}).filter(Boolean);if(!names.length){output.textContent='请先输入至少一个图层名。';return;}var ordered=this.rules.map(function(r,i){return {r:r,i:i};}).sort(function(a,b){return (Number(b.r.priority)||0)-(Number(a.r.priority)||0)||a.i-b.i;});output.textContent=names.map(function(name){var hits=ordered.filter(function(x){return self.matches(x.r,name);}).map(function(x){var r=x.r;return (r.name||('#'+(x.i+1)))+' → '+[r.parentGroup,r.parentClass,r.tagText].filter(Boolean).join(' / ');});return name+'\\n  '+(hits.length?hits.join('\\n  '):'未命中规则（仍会由名称提取器分析）');}).join('\\n\\n');};

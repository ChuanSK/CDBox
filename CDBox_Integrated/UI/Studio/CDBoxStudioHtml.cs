...
{{SECTION_DRAWING_SCRIPT}}
  ...
  function buildSettingsPayload(){var theme=(document.querySelector('input[name=studioTheme]:checked')||{}).value||'light';var animations=document.getElementById('animationsToggle').checked?'1':'0';var promptInstall=document.getElementById('promptInstallToggle').checked?'1':'0';var channel=(document.getElementById('updateChannel')||{}).value||'studio-preview';var source=(document.getElementById('updateSourceUrl')||{}).value||'';return 'theme='+encodeURIComponent(theme)+'&animations='+animations+'&promptInstall='+promptInstall+'&updateChannel='+encodeURIComponent(channel)+'&updateSourceUrl='+encodeURIComponent(source);}

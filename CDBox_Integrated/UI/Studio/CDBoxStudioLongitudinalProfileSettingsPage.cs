    ...
    internal static class CDBoxStudioLongitudinalProfileSettingsPage
    {
            ...
            CDBoxStudioSettings studio)
        {
            ...
            return "<section id=\"longitudinalProfileSettingsPage\" style=\"display:none;height:calc(100vh - 48px);min-height:600px\">"
                + "<iframe id=\"longitudinalProfileSettingsFrame\" title=\"?????\" style=\"display:block;width:100%;height:100%;border:0;background:transparent\" srcdoc=\""
            ...
            CDBoxStudioSettings studio, bool embedded)
        {
            ...
            html.Append("<!doctype html><html lang=\"zh-CN\"><head>")
                ...
                .Append("\"><main class=\"page\"><header><h1>?????</h1>")
                .Append("<button id=\"save\" class=\"primary\">????</button></header>")
                .Append("<section class=\"card\"><h2>?????</h2><div class=\"form-grid\">")
                .Append(Field("????", "headerWidth", "number", "0.1"))
                .Append(Field("????????", "headerGap", "number", "0.1"))
                .Append(Field("????", "horizontalScale", "number", "1"))
                .Append(Field("????", "verticalScale", "number", "1"))
                .Append(Field("??????", "horizontalGrid", "number", "0.1"))
                .Append(Field("??????", "elevationGrid", "number", "0.1"))
                .Append(Field("??????", "elevationPadding", "number", "0.1"))
                .Append(Field("????", "layerName", "text", null))
                .Append("</div></section>")
                .Append("<section class=\"card\"><h2>????</h2><div class=\"form-grid\">")
                .Append(Field("????", "headerTextStyle", "text", null))
                .Append(Field("????", "headerTextHeight", "number", "0.1"))
                .Append(Field("?????ACI?", "headerTextColor", "number", "1"))
                .Append("<label class=\"field\"><span>????</span><div class=\"segmented\" id=\"alignment\">")
                .Append("<button type=\"button\" data-value=\"???\">???</button>")
                .Append("<button type=\"button\" data-value=\"????\">????</button>")
                .Append("<button type=\"button\" data-value=\"???\">???</button>")
                .Append("</div></label>")
                .Append(Field("?????", "elevationDecimals", "number", "1"))
                .Append(Field("?????", "valueDecimals", "number", "1"))
                .Append(Field("?????", "slopeDecimals", "number", "1"))
                .Append("</div></section>")
                .Append("<section class=\"card\"><h2>???</h2>")
                .Append("<div class=\"table-wrap\"><table><thead><tr><th>???</th><th>???</th><th>????</th><th>????</th></tr></thead><tbody id=\"rows\"></tbody></table></div></section>")
                .Append("<section class=\"card\"><h2>????</h2>")
                .Append("<div class=\"table-wrap\"><table><thead><tr><th>??</th><th>???ACI?</th><th>??</th><th>????</th><th>??</th></tr></thead><tbody id=\"styles\"></tbody></table></div></section>")
...
function render(data){
 ...
 document.querySelectorAll('#alignment button').forEach(function(button){button.classList.toggle('selected',button.dataset.value===(state.HeaderTextAlignment||'????'))});
...
window.CDBoxLongitudinalProfileSettingsSaved=function(data){render(data);toast('?????','success')};
window.CDBoxLongitudinalProfileSettingsError=function(message){toast(message||'????','error')};

...
{
    /// <summary>
    /// 保存图框流程选择浮窗的最后位置，跨 CAD 会话继续使用。
    /// </summary>
    internal static class CDBoxStudioFrameChoicePositionStore
    {
        private const string FolderName = "CDBox";
        private const string FileName = "FrameChoiceWindow.position";
        ...
        public static bool TryLoad(out double left, out double top)
        {
            ...
            try
            {
                ...
                return double.TryParse(lines[0], NumberStyles.Float,
                           CultureInfo.InvariantCulture, out left)
                    && double.TryParse(lines[1], NumberStyles.Float,

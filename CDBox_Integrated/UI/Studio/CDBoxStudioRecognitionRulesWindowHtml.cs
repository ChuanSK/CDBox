...
");
        ...
        private static string BuildEditorCard(bool standalone, string logFilePath)
        {
            ...
            page.Append("<div class=\"rr-card-head\"><div class=\"rr-title\"><strong>规则表</strong><em>从上到下匹配，启用的规则才参与自动识别。</em></div><div class=\"rr-top-tools\"><div class=\"rr-search-wrap\" id=\"searchWrap\"><input id=\"rulesSearch\" class=\"rr-search\" placeholder=\"搜索：匹配内容、父属性、分类、标签...\" /><button id=\"toggleSearch\" class=\"rr-search-icon\" title=\"搜索 / Ctrl+F\">🔍</button></div></div></div>");
            page.Append("<div class=\"rr-toolbar\"><div class=\"rr-left\"><button id=\"addRuleButton\" class=\"rr-btn\">新增</button><button id=\"deleteRuleButton\" class=\"rr-btn danger\">删除</button><button id=\"moveRuleUpButton\" class=\"rr-btn\">上移</button><button id=\"moveRuleDownButton\" class=\"rr-btn\">下移</button><button id=\"restoreDefaultRulesButton\" class=\"rr-btn warning\">恢复默认</button><button id=\"saveRulesButton\" class=\"rr-btn primary\">保存</button></div><div class=\"rr-actions\">");
            ...
            page.Append("<div class=\"rr-tips\"><strong>填写说明：</strong><span>匹配方式：精确=完全一致；包含=图层名含有该文字；通配符支持 * 和 ?；正则适合复杂规则。</span><span>标签：可填写管径、材质、恢复方式等关键词，多个标签建议用 /、空格或中文顿号分隔，便于后续搜索和自动识别。</span><span>命中停止：同一图层命中本条后不再继续匹配下方规则。</span></div>");
            page.Append("<div class=\"rr-path\">保存位置：").Append(Html(CDBoxStudioRecognitionRules.RulesFilePath));
            if (standalone) page.Append("<br>Studio 日志：").Append(Html(logFilePath ?? string.Empty));
...
(function(w){
  ...
  RecognitionRulesPage.prototype.moveRule=function(offset){var target=this.selectedIndex+offset;if(this.selectedIndex<0||target<0||target>=this.rules.length)return;var item=this.rules.splice(this.selectedIndex,1)[0];this.rules.splice(target,0,item);this.selectedIndex=target;this.render();};

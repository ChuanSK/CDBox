...
namespace TCPipeAutoDraw.UI.Studio
{
    internal static class CDBoxStudioSettingsPage
    {
        public static string BuildStandaloneDocument()
        {
            ...
            html.Append("<!doctype html><html lang=\"zh-CN\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>CDBox??</title><style>");
            ...
            html.Append("</style></head><body data-theme=\"").Append(A(settings.Theme)).Append("\" class=\"").Append(settings.AnimationsEnabled ? string.Empty : "no-animations").Append("\"><main class=\"page\"><header class=\"head\"><h1>CDBox??</h1></header><div class=\"grid\">");
            html.Append("<section class=\"card wide\"><h2>??</h2><div class=\"themes\">");
            Theme(html, "light", "????", settings.Theme); Theme(html, "fresh", "????", settings.Theme); Theme(html, "dark", "????", settings.Theme);
            html.Append("</div><div class=\"actions\"><label class=\"switch\"><input id=\"animations\" type=\"checkbox\"").Append(settings.AnimationsEnabled ? " checked" : string.Empty).Append(">??????</label></div></section>");
            html.Append("<section class=\"card\"><h2>????</h2><label class=\"switch\"><input id=\"promptInstall\" type=\"checkbox\"").Append(app.PromptInstallOnLoad ? " checked" : string.Empty).Append(">????????????</label></section>");
            html.Append("<section class=\"card\"><h2>????</h2><label class=\"field\">CAD ??????<select id=\"colorOutputMode\">");
            ColorOutputOption(html, CDBoxColorOutputMode.PreserveOriginalType, "?????", settings.ColorOutputMode);
            ColorOutputOption(html, CDBoxColorOutputMode.PreferIndexColor, "??????", settings.ColorOutputMode);
            ColorOutputOption(html, CDBoxColorOutputMode.PreferTrueColor, "?????", settings.ColorOutputMode);
            ColorOutputOption(html, CDBoxColorOutputMode.CDBoxStandard, "CDBox ????", settings.ColorOutputMode);
            html.Append("</select></label><p class=\"hud-note\">???????? ACI / ??????????? Color Book ?????????</p></section>");
            html.Append("<section class=\"card wide\"><h2>?????</h2><div class=\"hud-grid\">");
            HudRange(html, "hudNormalOpacity", "hudNormalOpacityValue", "?????", settings.AnnotationHudNormalOpacity, true);
            HudRange(html, "hudHoverOpacity", "hudHoverOpacityValue", "???????", settings.AnnotationHudHoverOpacity, true);
            html.Append("<div class=\"hud-field\"><span>????</span><label class=\"switch\"><input id=\"hudGlowEnabled\" type=\"checkbox\"").Append(settings.AnnotationHudGlowEnabled ? " checked" : string.Empty).Append(">????????</label></div>");
            HudRange(html, "hudGlowIntensity", "hudGlowIntensityValue", "????", settings.AnnotationHudGlowIntensity, false);
            html.Append("<div class=\"hud-field\"><span>??????</span><label class=\"switch\"><input id=\"doubleClickOpen\" type=\"checkbox\"").Append(settings.DoubleClickOpenEnabled ? " checked" : string.Empty).Append(">??????????????</label></div>");
            html.Append("</div><p class=\"hud-note\">?????????????????????????????? 0% ??????????100% ???????????????</p></section>");
            html.Append("<section class=\"card\"><h2>????</h2><strong>").Append(CDBoxInstaller.IsInstalled() ? "???" : "???").Append("</strong></section>");
            html.Append("<section class=\"card wide\"><h2>???????</h2><div class=\"paths\"><label>CAD ??<code>").Append(H(CDBoxInstaller.GetCadDirectory())).Append("</code></label><label>????<code>").Append(H(CDBoxInstaller.GetInstallRoot())).Append("</code></label></div><div class=\"actions\"><button class=\"primary\" data-route=\"installPlugin\">??/??????</button><button data-route=\"localUpdatePlugin\">????</button><button class=\"danger\" data-route=\"uninstallPlugin\">??????</button><button data-route=\"openLogs\">??????</button></div></section>");
            html.Append("<section class=\"card wide\"><h2>????</h2><p class=\"hud-note\">????????????????????????????????????</p><div class=\"actions\"><button class=\"primary\" id=\"check\">????</button><button id=\"download\" disabled>???????</button></div><div id=\"progress\" class=\"progress\"></div><div id=\"result\" class=\"result\">??????</div></section>");
            html.Append("</div></main><script>");
            html.Append(@"(function(){var latest=null;function post(n,a){if(window.chrome&&chrome.webview)chrome.webview.postMessage('studio|'+n+'|'+encodeURIComponent(a||''));}function fraction(id){return (Number(document.getElementById(id).value||0)/100).toFixed(2);}function opacity(id){return (0.2+Number(document.getElementById(id).value||0)*0.008).toFixed(2);}function payload(){var t=(document.querySelector('input[name=theme]:checked')||{}).value||'light';return 'theme='+encodeURIComponent(t)+'&animations='+(document.getElementById('animations').checked?'1':'0')+'&promptInstall='+(document.getElementById('promptInstall').checked?'1':'0')+'&colorOutputMode='+encodeURIComponent(document.getElementById('colorOutputMode').value)+'&hudNormalOpacity='+opacity('hudNormalOpacity')+'&hudHoverOpacity='+opacity('hudHoverOpacity')+'&hudGlowEnabled='+(document.getElementById('hudGlowEnabled').checked?'1':'0')+'&hudGlowIntensity='+fraction('hudGlowIntensity')+'&doubleClickOpen='+(document.getElementById('doubleClickOpen').checked?'1':'0');}function save(){var t=(document.querySelector('input[name=theme]:checked')||{}).value||'light';document.body.dataset.theme=t;document.body.classList.toggle('no-animations',!document.getElementById('animations').checked);post('settings',payload());}document.querySelectorAll('input[name=theme],#animations,#promptInstall,#hudGlowEnabled,#doubleClickOpen,#colorOutputMode').forEach(function(x){x.addEventListener('change',save);});[['hudNormalOpacity','hudNormalOpacityValue'],['hudHoverOpacity','hudHoverOpacityValue'],['hudGlowIntensity','hudGlowIntensityValue']].forEach(function(pair){var input=document.getElementById(pair[0]),output=document.getElementById(pair[1]);input.addEventListener('input',function(){output.textContent=input.value+'%';});input.addEventListener('change',save);});document.querySelectorAll('[data-route]').forEach(function(x){x.addEventListener('click',function(){post(x.dataset.route,'');});});document.getElementById('check').onclick=function(){latest=null;document.getElementById('download').disabled=true;document.getElementById('progress').className='progress show';document.getElementById('progress').textContent='??????';post('checkUpdate',payload());};document.getElementById('download').onclick=function(){post('downloadUpdate',payload());};window.CDBoxStudioUpdateProgress=function(d){document.getElementById('progress').className='progress show';document.getElementById('progress').textContent=(d.message||'????')+(d.percent?' ? '+d.percent+'%':'');};window.CDBoxStudioUpdateResult=function(d){latest=d;document.getElementById('progress').className='progress';document.getElementById('result').textContent=d.success?(d.updateAvailable?'??????'+d.latestVersion:'????????'):'???????????????';document.getElementById('download').disabled=!(d.success&&d.updateAvailable);};window.CDBoxStudioDownloadResult=function(d){document.getElementById('progress').className='progress';document.getElementById('result').textContent=d.success&&d.verified?'?????????? CAD ????':'?????????????????';};window.CDBoxStudioToast=function(m,k){var x=document.createElement('div');x.className='toast';x.textContent=m||'';document.body.appendChild(x);setTimeout(function(){x.remove();},2800);};setTimeout(function(){post('ready','settings');},50);})();");

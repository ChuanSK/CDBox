...
")
        ...
        private static string BuildScript()
        {
            return @"
(function(){
var ctx=window.CDBoxParcelSurveyContext||{},draft=clone(ctx.Record||{}),viewKey='cdbox.parcel-editor.view.'+(ctx.Record&&ctx.Record.Id||'current'),view=loadView(),activeTab=Math.min(7,Math.max(1,Number(view.activeTab)||1));
...
function init(){ensure();renderTop();renderTabs();renderContent();restoreView();document.getElementById('tabs').onclick=function(e){var b=e.target.closest('[data-tab]');if(!b)return;syncAll();captureView();activeTab=Number(b.dataset.tab);view.scrollY=0;saveView();renderTabs();renderContent();window.scrollTo(0,0);};document.getElementById('saveParcel').onclick=function(){syncAll();captureView();post('saveParcel',JSON.stringify(draft));};document.getElementById('checkData').onclick=function(){syncAll();captureView();post('checkParcel',JSON.stringify(draft));};document.getElementById('exportSurvey').onclick=function(){syncAll();captureView();post('exportParcel',JSON.stringify(draft));};window.CDBoxStudioToast=toast;setTimeout(function(){post('ready','parcel-survey-editor');},50);}
init();
})();
";
        }

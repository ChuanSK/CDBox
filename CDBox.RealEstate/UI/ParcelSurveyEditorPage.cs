    ...
    public static class ParcelSurveyEditorPage
    {
            ...
            ParcelBoundaryCadService cad)
        {
            ...
            return new CDBoxPageDefinition(PageId, "宗地调查数据编辑器",
                ...
                {
                    return Route(request, store, cad);
                })
            {
                Width = 1380,
                Height = 900,
...
@media(max-width:1180px){.top-row{flex-wrap:wrap}.tabs{top:160px}.field-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.metrics{order:3}.content{padding-inline:14px}}@media(max-width:760px){.top{padding:12px}.tabs{top:220px;padding-inline:12px}.field-grid,.modal-grid{grid-template-columns:1fr}.field-card.wide,.modal-field.full{grid-column:auto}.intro{flex-direction:column}.actions{flex-wrap:wrap}}
")
                ...
                .Append("</div></header><nav id=\"tabs\" class=\"tabs\"></nav><section id=\"content\" class=\"content\"></section></main><div id=\"modalHost\"></div>")
            ...
            try
            {
                ...
                if (name == "cadrecognizeboundary")
                {
                    ParcelBoundaryCadSelection selection =
                        cad.SelectOwnershipBoundary();
                    if (selection != null)
                        result.ExecuteScript =
                            "window.CDBoxParcelCadBoundarySelected && window.CDBoxParcelCadBoundarySelected("
                            + SafeJson(selection) + ");";
                    return result;
                }
                if (name == "cadselectboundarysegment"
                    || name == "cadselectsignaturegroup")
                {
                    bool signature = name == "cadselectsignaturegroup";
                    ParcelBoundaryRangeSelection selection =
                        cad.SelectBoundaryRange(record, signature);
                    if (selection != null)
                    {
                        string callback = signature
                            ? "CDBoxParcelCadSignatureSelected"
                            : "CDBoxParcelCadSegmentSelected";
                        result.ExecuteScript = "window." + callback
                            + " && window." + callback + "("
                            + SafeJson(selection) + ");";
                    }
                    return result;
...
function renderBoundary(){var b=draft.Boundary,v=ctx.Validation||{},source=b.SourceObjectHandle?`<div class='source-meta'><span>图元 ${esc(b.SourceObjectHandle)}</span><span>图层 ${esc(b.SourceLayerName||'未命名')}</span><span>${b.SourceClockwise?'顺时针':'逆时针'}编号</span><span>面积 ${esc(b.SourceArea==null?'--':b.SourceArea)} ㎡</span></div>`:'';return `<section class='group'><div class='group-head'><div><strong>CAD 宗地几何</strong>${source}</div><small>保留手工添加；坐标可由权属线节点自动读取</small></div><div class='section-tools'><label class='closed'><input id='boundaryClosed' type='checkbox'${b.ParcelBoundaryClosed?' checked':''}> 宗地权属线已闭合</label><div><button class='mini' data-cad='boundary' type='button'>从图纸识别权属线</button> <button class='mini' data-add='point' type='button'>+ 手工添加界址点</button></div></div>${pointTable()}</section><section class='group'><div class='group-head'><strong>界址段列表</strong><small>界址标示表每页最多 26 段</small></div><div class='section-tools'><p>通过预览引线选择已识别起终点，也可继续手工录入。</p><div><button class='mini' data-cad='segment' type='button'>从图纸选择界址段</button> <button class='mini' data-add='segment' type='button'>+ 手工添加界址段</button></div></div>${segmentTable()}<div class='pages'>预计生成界址标示表 ${v.BoundarySegmentPageCount||1} 页；超过 26 段自动生成续表。</div></section><section class='group'><div class='group-head'><strong>界址签章分组</strong><small>同一邻宗的连续界址段自动合并，每页最多 13 组</small></div><div class='section-tools'><p>可在图中选择起终点后填写邻宗权利人、指界人和签章状态。</p><div><button class='mini' data-cad='signature' type='button'>从图纸选择起终点</button> <button class='mini' data-auto-signature='1' type='button'>按邻宗自动分组</button> <button class='mini' data-add='signature' type='button'>+ 手工添加签章组</button></div></div>${signatureTable()}<div class='pages'>预计生成界址签章表 ${v.SignatureGroupPageCount||1} 页；超过 13 组自动续表。</div></section><section class='group'><div class='group-head'><strong>自动说明</strong><small>根据权属线、界址段类别/位置及相邻宗地生成可编辑初稿</small></div><div class='section-tools'><p>重新生成会覆盖当前界址点位说明、界址线走向说明和宗地四至，并将状态设为“自动”。</p><button class='mini' data-generate-description='1' type='button'>生成界址说明与宗地四至</button></div></section>`+groupsFor(draft.Fields,ctx.Fields,'parcel','');}
...
var pendingBoundarySelection=null;
function showModal(body){var host=document.getElementById('modalHost');host.innerHTML=`<div class='modal-mask'><section class='modal'>${body}</section></div>`;host.querySelectorAll('[data-modal-close]').forEach(function(close){close.onclick=closeModal;});var first=host.querySelector('input:not([type=hidden]),select,textarea');if(first)setTimeout(function(){first.focus();},30);}
function closeModal(){document.getElementById('modalHost').innerHTML='';pendingBoundarySelection=null;}
function modalHeader(title,subtitle){return `<header class='modal-head'><div><h2>${esc(title)}</h2><p>${esc(subtitle||'')}</p></div><button class='mini' data-modal-close type='button'>关闭</button></header>`;}
function openBoundaryNumberStep(data){pendingBoundarySelection=clone(data);showModal(modalHeader('设置起点界址点号','已在图纸中选定起点，下一步确定编号方向。')+`<div class='modal-body'><div class='range-summary'>已识别 ${data.Vertices.length} 个权属线节点；应用后将替换当前界址点，并清空与旧点号相关的界址段和签章组。</div><label class='modal-field'><span>起点界址点号</span><input id='cadStartPointNumber' class='control' value='J1' placeholder='例如 J1'></label></div><footer class='modal-foot'><button class='btn' data-modal-close type='button'>取消</button><button id='boundaryNumberNext' class='btn primary' type='button'>下一步：选择方向</button></footer>`);document.getElementById('boundaryNumberNext').onclick=function(){var number=document.getElementById('cadStartPointNumber').value.trim(),match=/^(.*?)(\d+)$/.exec(number);if(!match||!match[1]){toast('起点界址点号必须以数字结尾，例如 J1。','error');return;}openBoundaryDirectionStep(match[1],Number(match[2]));};}
function openBoundaryDirectionStep(prefix,startNumber){var data=pendingBoundarySelection;showModal(modalHeader('选择界址点编号方向','点号将从 '+prefix+startNumber+' 开始连续生成。')+`<div class='modal-body'><div class='choice-grid'><label class='choice'><input type='radio' name='cadDirection' value='clockwise' checked><span><strong>顺时针</strong><br><small>沿权属线顺时针依次编号</small></span></label><label class='choice'><input type='radio' name='cadDirection' value='counterclockwise'><span><strong>逆时针</strong><br><small>沿权属线逆时针依次编号</small></span></label></div></div><footer class='modal-foot'><button id='boundaryDirectionBack' class='btn' type='button'>上一步</button><button id='applyBoundaryRecognition' class='btn primary' type='button'>应用并填入界址点</button></footer>`);document.getElementById('boundaryDirectionBack').onclick=function(){openBoundaryNumberStep(data);document.getElementById('cadStartPointNumber').value=prefix+startNumber;};document.getElementById('applyBoundaryRecognition').onclick=function(){var selected=document.querySelector('input[name=cadDirection]:checked'),clockwise=!selected||selected.value==='clockwise';applyBoundaryRecognition(data,prefix,startNumber,clockwise);};}
function applyBoundaryRecognition(data,prefix,startNumber,clockwise){var vertices=data.Vertices||[],n=vertices.length,start=vertices.findIndex(function(v){return Number(v.SourceIndex)===Number(data.SelectedStartSourceIndex);});if(start<0||n<3){toast('权属线节点数据无效，请重新识别。','error');return;}var step=clockwise===!!data.NativeClockwise?1:-1,points=[];for(var i=0;i<n;i++){var raw=(start+step*i+n*4)%n,v=vertices[raw],next=(raw+step+n)%n,edge=step===1?v.DistanceToNext:vertices[next].DistanceToNext;points.push({Sequence:i+1,PointNumber:prefix+(startNumber+i),X:v.X,Y:v.Y,DistanceToNext:edge,MarkerType:'',Description:'',Confirmed:false,Status:0});}draft.Boundary.Points=points;draft.Boundary.Segments=[];draft.Boundary.SignatureGroups=[];draft.Boundary.ParcelBoundaryClosed=true;draft.Boundary.SourceObjectHandle=data.SourceObjectHandle||'';draft.Boundary.SourceLayerName=data.SourceLayerName||'';draft.Boundary.SourceClockwise=clockwise;draft.Boundary.SourceArea=data.Area;var area=value('land.parcelArea');area.NumericValue=data.Area;area.TextValue='';area.Status=0;area.Confirmed=false;closeModal();renderContent();toast('已自动填入 '+n+' 个界址点及坐标，面积同步为 '+data.Area+' ㎡。','success');post('autoGenerateBoundaryDescriptions',JSON.stringify(draft));}
function openSegmentModal(range){var old=draft.Boundary.Segments.find(function(x){return x.StartPointNumber===range.StartPointNumber&&x.EndPointNumber===range.EndPointNumber;})||{};showModal(modalHeader('填写界址段信息','已通过图纸选择起点和终点，距离按编号方向的权属线真实边长汇总。')+`<div class='modal-body'><div class='range-summary'>${esc(range.StartPointNumber)} → ${range.MiddlePointNumbers?esc(range.MiddlePointNumbers)+' → ':''}${esc(range.EndPointNumber)} · ${esc(range.Distance)} m · ${esc(range.Direction)}方向</div><div class='modal-grid'><label class='modal-field'><span>界址线类别 *</span><select id='cadLineCategory' class='control'>${optionList(ctx.LineCategories,old.LineCategory||'')}</select></label><label class='modal-field'><span>界址线位置 *</span><select id='cadLinePosition' class='control'>${optionList(ctx.LinePositions,old.LinePosition||'待确认')}</select></label><label class='modal-field'><span>相邻宗地代码</span><input id='cadNeighborCode' class='control' value='${attr(old.NeighborParcelCode||'')}'></label><label class='modal-field'><span>相邻宗地权利人或地物</span><input id='cadNeighborOwner' class='control' value='${attr(old.NeighborOwner||'')}' placeholder='例如 李老七、2 米巷道'></label><label class='modal-field full'><span>段说明</span><textarea id='cadSegmentDescription' class='control'>${esc(old.Description||'')}</textarea></label><label class='choice'><input id='cadNeighborHandled' type='checkbox'${old.NeighborHandled?' checked':''}><span>相邻宗未识别项已处理</span></label><label class='choice'><input id='cadSegmentConfirmed' type='checkbox'${old.Confirmed?' checked':''}><span>本段信息已确认</span></label></div></div><footer class='modal-foot'><button class='btn' data-modal-close type='button'>取消</button><button id='applyCadSegment' class='btn primary' type='button'>填入界址段</button></footer>`);document.getElementById('applyCadSegment').onclick=function(){var category=document.getElementById('cadLineCategory').value,position=document.getElementById('cadLinePosition').value;if(!category){toast('请选择界址线类别。','error');return;}if(!position||position==='待确认'){toast('请选择已确认的界址线位置。','error');return;}var segment={StartPointNumber:range.StartPointNumber||'',MiddlePointNumbers:range.MiddlePointNumbers||'',EndPointNumber:range.EndPointNumber||'',Distance:Number(range.Distance)||0,LineCategory:category,LinePosition:position,NeighborParcelCode:document.getElementById('cadNeighborCode').value.trim(),NeighborOwner:document.getElementById('cadNeighborOwner').value.trim(),Direction:range.Direction||'',Description:document.getElementById('cadSegmentDescription').value.trim(),Confirmed:document.getElementById('cadSegmentConfirmed').checked,NeighborHandled:document.getElementById('cadNeighborHandled').checked,Status:3};upsertSegment(segment);closeModal();renderContent();post('autoGenerateBoundaryDescriptions',JSON.stringify(draft));};}
function upsertSegment(segment){draft.Boundary.Segments=draft.Boundary.Segments.filter(function(x){return !(x.StartPointNumber===segment.StartPointNumber&&x.EndPointNumber===segment.EndPointNumber);});draft.Boundary.Segments.push(segment);var order={};draft.Boundary.Points.forEach(function(p,i){order[p.PointNumber]=i;});draft.Boundary.Segments.sort(function(a,b){return (order[a.StartPointNumber]??99999)-(order[b.StartPointNumber]??99999);});}
function openSignatureModal(range){var today=new Date().toISOString().slice(0,10),old=draft.Boundary.SignatureGroups.find(function(x){return x.StartPointNumber===range.StartPointNumber&&x.EndPointNumber===range.EndPointNumber;})||{},segment=draft.Boundary.Segments.find(function(x){return x.StartPointNumber===range.StartPointNumber&&x.EndPointNumber===range.EndPointNumber;})||{},neighborOwner=old.NeighborOwner||segment.NeighborOwner||'',neighborCode=old.NeighborParcelCode||segment.NeighborParcelCode||'',parcelRepresentative=old.ParcelRepresentative||value('rights.ownerName').TextValue||'';showModal(modalHeader('填写界址签章组信息','已通过图纸选择签章界址线起终点；系统不会自动伪造签章。')+`<div class='modal-body'><div class='range-summary'>起点 ${esc(range.StartPointNumber)} · 中间点 ${esc(range.MiddlePointNumbers||'/')} · 终点 ${esc(range.EndPointNumber)}</div><div class='modal-grid'><label class='modal-field'><span>相邻宗地权利人</span><input id='sigNeighborOwner' class='control' value='${attr(neighborOwner)}'></label><label class='modal-field'><span>相邻宗地代码</span><input id='sigNeighborCode' class='control' value='${attr(neighborCode)}'></label><label class='modal-field'><span>邻宗指界人姓名</span><input id='sigNeighborRepresentative' class='control' value='${attr(old.NeighborRepresentative||'')}'></label><label class='modal-field'><span>本宗地指界人姓名</span><input id='sigParcelRepresentative' class='control' value='${attr(parcelRepresentative)}'></label><label class='modal-field'><span>指界日期</span><input id='sigDate' class='control' type='date' value='${attr(old.ConfirmationDate||today)}'></label><label class='modal-field'><span>签章状态</span><select id='sigStatus' class='control'>${optionList(['待签章','已签章','无需签章'],old.SignatureStatus||'待签章',false)}</select></label><label class='choice'><input id='sigPaperBlank' type='checkbox'${old.PreservePaperSignatureBlank===false?'':' checked'}><span>保留空白供纸质签章</span></label><label class='choice'><input id='sigConfirmed' type='checkbox'${old.Confirmed?' checked':''}><span>签章组信息已确认</span></label></div></div><footer class='modal-foot'><button class='btn' data-modal-close type='button'>取消</button><button id='applyCadSignature' class='btn primary' type='button'>填入签章组</button></footer>`);document.getElementById('applyCadSignature').onclick=function(){var group={StartPointNumber:range.StartPointNumber||'',MiddlePointNumbers:range.MiddlePointNumbers||'',EndPointNumber:range.EndPointNumber||'',NeighborOwner:document.getElementById('sigNeighborOwner').value.trim(),NeighborParcelCode:document.getElementById('sigNeighborCode').value.trim(),NeighborRepresentative:document.getElementById('sigNeighborRepresentative').value.trim(),ParcelRepresentative:document.getElementById('sigParcelRepresentative').value.trim(),ConfirmationDate:document.getElementById('sigDate').value,SignatureStatus:document.getElementById('sigStatus').value,PreservePaperSignatureBlank:document.getElementById('sigPaperBlank').checked,Confirmed:document.getElementById('sigConfirmed').checked,Status:3};draft.Boundary.SignatureGroups=draft.Boundary.SignatureGroups.filter(function(x){return !(x.StartPointNumber===group.StartPointNumber&&x.EndPointNumber===group.EndPointNumber);});draft.Boundary.SignatureGroups.push(group);closeModal();renderContent();};}
window.CDBoxParcelCadBoundarySelected=openBoundaryNumberStep;
window.CDBoxParcelCadSegmentSelected=openSegmentModal;
window.CDBoxParcelCadSignatureSelected=openSignatureModal;
window.CDBoxParcelDescriptionsGenerated=function(record){draft=clone(record||draft);ensure();renderTop();renderContent();};

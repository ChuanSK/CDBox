...
@media(max-width:1180px){.shell{grid-template-columns:224px minmax(0,1fr)}.field-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.content{padding-inline:14px}}@media(max-width:760px){.shell{grid-template-columns:190px minmax(0,1fr)}.top{align-items:flex-start;flex-direction:column;padding:14px}.actions{flex-wrap:wrap}.field-grid,.modal-grid{grid-template-columns:1fr}.field-card.wide,.modal-field.full{grid-column:auto}}
")
                .Append("</style></head><body><main class=\"shell page\" data-page=\"")
                .Append(PageId).Append("\"><aside class=\"sidebar\"><div class=\"brand\"><div class=\"logo\">地</div><h1>宗地调查</h1></div>")
                .Append("<label class=\"parcel-picker\"><strong>当前宗地</strong><select id=\"scopeRegion\"></select></label>")
                .Append("<div class=\"scope-actions\"><button class=\"mini\" data-scope-action=\"newparcel\" type=\"button\">权属线建宗</button><button class=\"mini\" data-scope-action=\"create\" type=\"button\">区域建宗</button><button class=\"mini wide\" data-scope-action=\"bind\" type=\"button\">选择区域建宗</button><button class=\"mini\" data-scope-bound-only=\"1\" data-scope-action=\"rename\" type=\"button\">重命名宗地</button><button class=\"mini danger\" data-scope-bound-only=\"1\" data-scope-action=\"delete\" type=\"button\">删除宗地信息</button></div><nav id=\"tabs\" class=\"nav\"></nav></aside>")
                .Append("<section class=\"workspace\"><header class=\"top\"><div class=\"identity\"><h2>宗地调查数据编辑器</h2><div id=\"currentParcel\" class=\"parcel\"></div></div><div class=\"actions\"><button id=\"saveParcel\" class=\"btn\" type=\"button\">保存宗地</button><button id=\"checkData\" class=\"btn\" type=\"button\">检查数据</button><button id=\"exportSurvey\" class=\"btn primary\" type=\"button\">导出调查表</button></div></header><section id=\"content\" class=\"content\"></section></section></main>")
...
function renderTop(){var p=canEdit?(scope.ParcelName||value('parcel.parcelSeaCode').TextValue||value('parcel.parcelCode').TextValue||'未命名宗地'):'尚未选择宗地';document.getElementById('currentParcel').textContent='当前宗地：'+p;['saveParcel','checkData','exportSurvey'].forEach(function(id){document.getElementById(id).disabled=!canEdit;});}

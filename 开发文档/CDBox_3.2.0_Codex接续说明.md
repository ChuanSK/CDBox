...
- 管长标注重新绑定其他对象后，上侧长度能够正常变化。
- 下侧注记中的长度可能仍保留原值，没有随新绑定对象同步。

该问题尚未在 3.2.0 中修改。后续应先确认上下两行文字的组成配置和存储字段，再检查：

- `PipeLengthAnnotationTextComposer.cs` 中主文字、次文字的长度令牌组合。
- `PipeLengthAnnotationObjectService.cs` 中重新绑定、刷新文字和保存元数据的流程。
- `PipeLengthAnnotationEditModels.cs` 中主次文字模型是否同时接收新对象长度。
- 旧标注迁移数据是否将下侧文字保存为固定文本，导致重新组合时没有替换长度。

...
namespace TCPipeAutoDraw.Modules.SectionDrawing
{
    ...
    public static class SectionBatchDrawingService
    {
        ...
        public static SectionBatchDrawingResult Run(Document doc, Action<int, int, string> progress)
        {
            ...
            // 交互选区完成后才启动读取进度。命令入口不再提前创建一个
            // 无法随选择取消而结束的“正在读取”会话。
            ReportProgress(progress, 0, 1000, "正在建立节点索引…");
            List<SectionBatchSourceData> sources = BuildPlanSources(doc, ids,
                progress);
            if (sources.Count == 0)
            {
                return new SectionBatchDrawingResult
            ...
            SectionDrawingOptions baseOptions =
                SectionDrawingSettingsStore.Load();
            List<SectionBatchPlanGroup> groups =
                WastewaterSectionDrawingRegistry.GetRequired()
                    .PlanBatch(sources, baseOptions);
            ReportProgress(progress, 500, 1000,
                "断面数据读取完成，请选择落图位置…");
            if (groups.Count == 0)
            {
                return new SectionBatchDrawingResult
            ...
            var mainGroups = new List<SectionBatchPlanGroup>();
            var branchGroups = new List<SectionBatchPlanGroup>();
            for (int i = 0; i < groups.Count; i++)
            {
                SectionBatchPlanGroup group = groups[i];
                if (group != null && group.IsBranch) branchGroups.Add(group);
            ...
            ReportProgress(progress, 500, 1000, "正在生成批量断面图…");

            // 断面实体及其绑定索引不会改变污水工程量。暂停工程量实时监视，
            // 防止数百个新实体在命令结束后触发一次无意义的全量同步。
            using (QuantityDashboardLiveMonitor.SuspendChangeTracking())
            {
            ...
            ReportProgress(progress, 1000, 1000, "批量断面图生成完成。");
        ...
        private static void DrawGroupRows(
            Document doc,
            IList<SectionBatchPlanGroup> groups,
            SectionDrawingOptions baseOptions,
            Point3d startPoint,
            ref double nextRowY,
            ...
            ref int progressIndex)
        {
            ...
            for (int i = 0; i < groups.Count; i++)
            {
                ...
                SectionDrawingOptions options = group.Options == null
                    ? baseOptions.Clone() : group.Options.Clone();
                ...
                SectionDrawingResult drawResult = SectionDrawingService.Draw(
                    doc, options, bodyOrigin, ResolveSourceIds(doc,
                        group.SourceIds));
                ...
                column++;
                progressIndex++;
                int overall = 500 + (int)Math.Round(progressIndex * 500.0
                    / Math.Max(progressTotal, 1));
                ReportProgress(progress, overall, 1000,
                    "正在生成批量断面图：" + progressIndex + "/" +
                    progressTotal);
            }
        ...
        /// <summary>
        /// CAD 适配层只读取对象句柄和属性快照；结构层转换、断面合并、
        /// 标题连续段合并及样式规划由 Wastewater 模块执行。
        /// </summary>
        private static List<SectionBatchSourceData> BuildPlanSources(
            Document doc, IList<ObjectId> ids,
            Action<int, int, string> progress)
        {
            var result = new List<SectionBatchSourceData>();
            if (doc == null || ids == null) return result;
            List<ObjectId> knownNodes = QuantityPipeAttributeService
                .FindSupportedNodeObjectIds(doc,
                    delegate(int current, int total, string message)
                    {
                        int overall = total <= 0 ? 0 :
                            (int)Math.Round(current * 200.0 / total);
                        ReportProgress(progress, overall, 1000,
                            "正在建立节点索引…");
                    });
            ...
            {
                ObjectId id = ids[i];
                ReportProgress(progress, 200 + (int)Math.Round(
                    (i + 1) * 280.0 / Math.Max(ids.Count, 1)), 1000,
                    "正在读取断面数据：" + (i + 1) + "/" + ids.Count);
                ...
                if (info == null || info.Attributes == null ||
                    !info.HasSavedAttributes) continue;
                result.Add(new SectionBatchSourceData
                {
                    SourceId = id.Handle.ToString(),
                    Order = i + 1,
                    Attributes = info.Attributes.Clone()
                });
            }
            ReportProgress(progress, 480, 1000, "正在合并相同断面…");
            return result;
        }

        private static IEnumerable<ObjectId> ResolveSourceIds(Document doc,
            IEnumerable<string> handles)
        {
            if (doc == null || doc.Database == null) yield break;
            foreach (string text in handles ?? new string[0])
            {
                long value;
                if (!long.TryParse((text ?? string.Empty).Trim(),
                        NumberStyles.HexNumber, CultureInfo.InvariantCulture,
                        out value)) continue;
                ObjectId id;
                try
                {
                    id = doc.Database.GetObjectId(false,
                        new Handle(value), 0);
                }
                catch { continue; }
                if (!id.IsNull) yield return id;
            }
        }

        private static List<BatchPipeSectionSource> BuildSources(Document doc,
            IList<ObjectId> ids, Action<int, int, string> progress)
        {
            var sources = new List<BatchPipeSectionSource>();
            if (doc == null || ids == null) return sources;

            List<ObjectId> knownNodeObjectIds =
                QuantityPipeAttributeService.FindSupportedNodeObjectIds(doc,
                    delegate(int current, int total, string message)
                    {
                        int overall = total <= 0 ? 0 :
                            (int)Math.Round(current * 200.0 / total);
            ...
            {
                ObjectId id = ids[sourceIndex];
                order++;
                int readOverall = 200 + (int)Math.Round(
                    (sourceIndex + 1) * 280.0 / Math.Max(ids.Count, 1));
                ReportProgress(progress, readOverall, 1000,
                    "正在读取断面数据：" + (sourceIndex + 1) + "/" +
                    ids.Count);
                if (id.IsNull) continue;

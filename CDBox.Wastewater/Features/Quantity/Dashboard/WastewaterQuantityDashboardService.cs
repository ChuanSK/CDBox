...
{
    public static class WastewaterQuantityDashboardService
    {
        public static QuantityDashboardContext BuildContext(QuantityDashboardRequest request)
        {
            ...
            {
                WastewaterQuantityDashboardRegionService.SaveScope(document,
                    request.scopeType, request.regionId);
                QuantityDashboardRequest verified =
                    WastewaterQuantityDashboardRegionService.GetSavedScope(document);
            ...
            {
                using (document.LockDocument()) context.regions = WastewaterQuantityDashboardRegionService.GetRegions(document);
            }
        ...
        public static QuantityDashboardSnapshot BuildSnapshot(QuantityDashboardRequest request, Action<int, int, string> progress)
        {
            ...
            WastewaterQuantityDashboardRegionService.SaveScope(doc, request.scopeType,
                request.regionId);
            ...
            {
                regionId = WastewaterQuantityDashboardRegionService.FindRegionObjectId(doc, request.regionId);
                if (regionId.IsNull) throw new InvalidOperationException("固定统计区域不存在或已被删除。");
                regionInfo = WastewaterQuantityDashboardRegionService.ReadRegion(doc, request.regionId, regionId);
            ...
            List<ObjectId> allSavedIds = WastewaterQuantityCadReader.FindObjectsWithSavedAttributes(doc);
            HashSet<string> scopedHandles;
            List<ObjectId> scopedIds = BuildScopedObjectIdList(doc, allSavedIds, regionId, out scopedHandles);
            Dictionary<string, WastewaterQuantityCadObject> infoByHandle = ReadScopedInfos(doc, scopedIds, scopedHandles, snapshot);

            Report(progress, 1, 6, "正在复用正式工程量计算逻辑……");
            QuantityCalculationReport report = WastewaterQuantityCalculationReportService.BuildReport(doc, scopedIds, null);
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                ...
                foreach (ObjectId id in ids ?? Enumerable.Empty<ObjectId>())
                {
                    try
                    {
                        ...
                        if (region != null && !WastewaterQuantityDashboardRegionService.IsEntityIncluded(entity, region)) continue;
                        result.Add(id);
        ...
        private static Dictionary<string, WastewaterQuantityCadObject> ReadScopedInfos(Document doc, IEnumerable<ObjectId> ids, HashSet<string> scopedHandles, QuantityDashboardSnapshot snapshot)
        {
            var result = new Dictionary<string, WastewaterQuantityCadObject>(StringComparer.OrdinalIgnoreCase);
            foreach (ObjectId id in ids ?? Enumerable.Empty<ObjectId>())
            {
                try
                {
                    WastewaterQuantityCadObject info = WastewaterQuantityCadReader.Read(doc, id);
        ...
        private static void AddWellRows(QuantityDashboardSnapshot snapshot, IEnumerable<QuantityWellCalculationRow> rows, IDictionary<string, WastewaterQuantityCadObject> infos)
        {
            int count = 0;
        ...
        private static void AddPipeRows(QuantityDashboardSnapshot snapshot, IEnumerable<QuantityMainPipeCalculationRow> rows, IDictionary<string, WastewaterQuantityCadObject> infos, bool branch)
        {
        ...
        private static void AddOtherObjects(QuantityDashboardSnapshot snapshot, Document doc, ObjectId regionId, HashSet<string> attributedHandles)
        {
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                ...
                foreach (ObjectId id in space)
                {
                    ...
                    if (region != null && !WastewaterQuantityDashboardRegionService.IsEntityIncluded(entity, region)) continue;
                    LayerMetadata metadata = WastewaterRuntimeServices
                        .LayerClassification == null ? new LayerMetadata()
                        : WastewaterRuntimeServices.LayerClassification
                            .GetLayerMetadata(db, tr, entity.Layer)
                            as LayerMetadata ?? new LayerMetadata();
        ...
        private static void BuildQualityIssues(QuantityDashboardSnapshot snapshot, IDictionary<string, WastewaterQuantityCadObject> infos)
        {
            foreach (QuantityDashboardDetailRow detail in snapshot.details)
            {
                WastewaterQuantityCadObject info;

...
{
    public static class WastewaterQuantityDashboardRegionService
    {
        ...
        public static QuantityDashboardRequest GetSavedScope(Document doc)
        {
            ...
            result.documentId = WastewaterQuantityDashboardService.GetDocumentId(doc);
            return result;
        }
        ...
        public static QuantityDashboardRegionInfo CreateRectangleRegion(Document doc)
        {
            ...
            using (WastewaterRuntimeServices.Prompts == null ? null
                : WastewaterRuntimeServices.Prompts.Begin("创建统计区域",
                    "指定工程量统计区域第一个角点；按 Esc 取消。"))
                p1 = ed.GetPoint("\n指定工程量统计区域第一个角点：");
            if (p1.Status != PromptStatus.OK) return null;
            PromptCornerOptions corner = new PromptCornerOptions("\n指定工程量统计区域对角点：", p1.Value);
            PromptPointResult p2;
            using (WastewaterRuntimeServices.Prompts == null ? null
                : WastewaterRuntimeServices.Prompts.Begin("创建统计区域",
                    "指定工程量统计区域对角点；按 Esc 取消。"))
                p2 = ed.GetCorner(corner);
        ...
        public static QuantityDashboardRegionInfo BindExistingClosedPolyline(Document doc)
        {
            ...
            using (WastewaterRuntimeServices.Prompts == null ? null
                : WastewaterRuntimeServices.Prompts.Begin("绑定统计区域",
                    "选择已有闭合多段线；按 Esc 取消。"))
                selected = ed.GetEntity(options);
        ...
        private static void EnsureRegionLayer(Database db, Transaction tr)
        {
            ...
            if (table != null && table.Has(RegionLayerName))
                id = table[RegionLayerName];
            else if (table != null)
            {
                table.UpgradeOpen();
                var created = new LayerTableRecord
                {
                    Name = RegionLayerName,
                    Color = Color.FromColorIndex(ColorMethod.ByAci, 8)
                };
                id = table.Add(created);
        ...
        private static void ClassifyRegionLayer()
        {
            if (WastewaterRuntimeServices.LayerClassification != null)
                WastewaterRuntimeServices.LayerClassification
                    .EnsureGeneratedLayerClassification(RegionLayerName,
                        "CDBox图层");
        }
        ...
        private static string PromptRegionName(Editor ed, string defaultName)
        {
            ...
            PromptResult result;
            using (WastewaterRuntimeServices.Prompts == null ? null
                : WastewaterRuntimeServices.Prompts.Begin("统计区域名称",
                    "输入统计区域名称；按 Enter 使用默认名称。"))
